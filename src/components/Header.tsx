...
import { Phone, ShieldCheck, Download, Search, X, Star } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm">
      ...
      <div className="bg-brand-700 text-white text-xs py-1.5 px-4 sm:px-6 lg:px-8 xl:px-10 text-center font-medium tracking-wide flex justify-between items-center max-w-[1920px] mx-auto">
        <span className="truncate">
          {BUSINESS_CONFIG.hideAddress ? `✨ Wholesale Catalogue • ${BUSINESS_CONFIG.tagline}` : `📍 Surat Wholesale Market • ${BUSINESS_CONFIG.tagline}`}
        </span>
        <div className="flex items-center gap-3 shrink-0">
          {BUSINESS_CONFIG.reviewUrl && (
            <a
              ...
              className="text-[11px] bg-white/10 hover:bg-white/20 text-white px-2.5 py-0.5 rounded-full font-bold transition-colors flex items-center gap-1 border border-white/20"
            >
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          {BUSINESS_CONFIG.reviewUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-300 px-3 py-2 rounded-xl text-xs font-bold transition-all shadow-sm"
              title="Write Google / Store Review"
            >
